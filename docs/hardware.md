...
hftool's recommended AMD runtime is the Docker image with ROCm 7.1.1 and
PyTorch 2.9.1. It uses the host's existing `amdgpu` kernel driver and does not
install or replace host ROCm userspace or gaming drivers.

| Task | Model | VRAM Required | Notes |
|------|-------|---------------|-------|
| Text-to-Image | SANA Sprint 1.6B | 10 GB minimum | Fast drafts/backgrounds |
| Text/Image-to-Image | FLUX.2 Klein 4B | 13 GB minimum | Apache-2.0 consumer-GPU option |
| Text-to-Image | Z-Image-Turbo | 16 GB minimum | Default general model |
| Image-to-Image | Qwen Image Edit | 24 GB minimum | Offload/sharding may be needed |
| Image-to-Image | FLUX.2 Klein 9B | 29 GB minimum | Gated, non-commercial |
...
```bash
# Auto-select using live free VRAM, model minimum, reserve, and display pressure
