...
def _gpu_reexec_if_needed():
    ...
    # Determine the platform without importing torch. Device visibility must
    # be finalized before anything imports it.
    is_rocm = bool(
        os.path.exists("/opt/rocm") or
        os.path.exists("/dev/kfd") or
    ...
    if gpu_value == "auto":
        ...
        try:
            ...
            if selected is None:
                return
            gpu_indices = [selected.index]
        except Exception:
            return
    else:
        try:
            gpu_indices = [int(value.strip()) for value in gpu_value.split(",")]
        except ValueError:
            return
    ...
    physical_devices = ",".join(str(index) for index in gpu_indices)
    renumbered_devices = ",".join(str(index) for index in range(len(gpu_indices)))
    if is_rocm:
        os.environ["ROCR_VISIBLE_DEVICES"] = physical_devices
        os.environ["HIP_VISIBLE_DEVICES"] = renumbered_devices
    else:
        os.environ["CUDA_VISIBLE_DEVICES"] = physical_devices
    os.environ["HFTOOL_PHYSICAL_GPU_INDICES"] = physical_devices
...
import click  # noqa: E402 - GPU visibility must be set before downstream imports

# Import shell completion functions
from hftool.core.completion import (  # noqa: E402
    complete_tasks,
    complete_models,
    complete_devices,
    complete_dtypes,
...
def main(
    ...
    if wizard:
        ...
        if not dry_run and not ensure_pytorch_ready():
            sys.exit(1)
