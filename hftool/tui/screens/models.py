...
class ModelBrowserScreen(Screen):
    ...
    def _load_all_models(self) -> None:
        ...
        table.add_columns("Task", "Model", "Use", "VRAM", "Catalog", "Local", "License")

        try:
            from hftool.core.models import MODEL_REGISTRY
            from hftool.core.download import get_model_download_status, get_models_disk_usage
    ...
    def _delete_model(self, task_name: str, short_name: str) -> None:
        try:
            ...
            if delete_model(info.repo_id):
                ...
                self.app.call_from_thread(self.notify, "Model not found on disk", severity="warning")
            self.app.call_from_thread(self._load_all_models)
        except Exception as e:
