...
A Docker-first terminal interface for running Hugging Face models, optimized for AMD ROCm.

> **What this is:** A full-screen TUI for common AI tasks, with a secondary CLI for scripting. Docker keeps the current supported ROCm/PyTorch userspace isolated from host graphics drivers.
>
...
|------|-------|--------|------|
| Text-to-Image | `t2i` | Z-Image, FLUX.2 Klein 4B, SANA Sprint, Qwen 2512 | [Guide](docs/text-to-image.md) |
| Image-to-Image | `i2i` | Qwen Image Edit, FLUX.2 Klein 4B/9B, SDXL | [Guide](docs/image-to-image.md) |
...
```bash
# Primary interface: full-screen TUI in the isolated Docker runtime
hftool tui

# Secondary interface: scriptable CLI
...
# Inspect the exact model defaults and live GPU choice without downloading
hftool -t t2i -m flux2-klein-4b -i "A cat" --dry-run
...
- **Load-aware GPU selection** — Uses live free VRAM, model requirements, display pressure, and physical GPU identity
- **Explicit Multi-GPU** — Model sharding is enabled only with `--gpu all` (or an explicit multi-device request)
